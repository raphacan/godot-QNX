...
def get_opts():
    ...
    return [
        ...
        BoolVariable("screen", "Enable QNX Screen support", True),
...
def configure(env: "SConsEnvironment"):
    ...
    qnx_host = get_env_qnx_host()
    if qnx_host == "":
        print_error("QNX_HOST environment variable not set!")
    qnx_target = get_env_qnx_target()
    if qnx_target == "":
        print_error("QNX_TARGET environment variable not set!")
    ...
    env.Prepend(CFLAGS=["-D_QNX_SOURCE"])
    ...
    env.Append(LIBS=["socket", "screen", "slog2"])
